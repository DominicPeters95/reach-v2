...
@Component({
  selector: 'app-fill-in-question',
  templateUrl: './fill-in-question.component.html',
  styleUrls: ['./fill-in-question.component.scss']
})

export class FillInQuestionComponent implements OnInit, OnChanges{
  @Input() option: any;
  contexts: any[] = [];
  questions: Question[] = [];
  prev = -1;
  
  constructor(private dataService: FillInService, private scroller: ViewportScroller){ }
    ...
    if(this.dataService.isBlankCLicked){
      if(!this.dataService.isAnswerClikced){
        ...
        if(this.contexts[i].content === undefined){
          ...
          setTimeout(()=>{this.contexts[i] = { isSelected: true};}, 10);
          this.dataService.index = i;
          this.dataService.qIndexList.push(i);
