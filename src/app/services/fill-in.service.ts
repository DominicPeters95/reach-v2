...
export class FillInService {
  ...
  loadData(){
    this.http.get('http://localhost:3000/fill_in').subscribe((result:any)=>{
      if(result){
  ...
  transformQuestion(question: Question): Question{
    ...
    content = content.replace(regex, function(match) {
      if (match === "<c-blank ") {
        return "<c-blank [filledAnswer]=\"context\" [subtype]=\"'fill_in'\" ";
      } else if(match === "ids='") {
        return "[ids]=\"'";
  ...
  reloadAll(){
    ...
    this.answerChecker = new Subject<boolean>();
  }
